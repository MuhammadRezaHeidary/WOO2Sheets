function mersa_add_processing() {

  var mersa_google_sheets = SpreadsheetApp.getActiveSpreadsheet();
  var mersa_processing = mersa_google_sheets.getSheetByName("WOO_Processing");
  ...
  const MERSA_INDEX = [MERSA_MAX_COL, MERSA_NOTE_COL, MERSA_MAX_ROW, MERSA_PROJECT_COL, MERSA_FNAME_COL, MERSA_LNAME_COL, MERSA_COMPANY_COL, MERSA_START_DATE_COL, MERSA_CONDITION_COL, MERSA_EDIT_DATE_COL, MERSA_BILLING_COL, MERSA_NUMBER_OF_ORDERS, MERSA_CONDITION_PROCESSING];

  mersa_get_completed_orders(mersa_processing ,mersa_orders, MERSA_INDEX);

  var mersa_sheet_dest = mersa_processing.getRange(1,1,MERSA_MAX_ROW,MERSA_MAX_COL);

  mersa_sheet_dest.setFontFamily('Vazirmatn');
  mersa_sheet_dest.setFontSize(10);
  mersa_processing.getRange(1,1,1,MERSA_MAX_COL).setFontColor('White');
  mersa_processing.getRange(1,1,1,MERSA_MAX_COL).setBackground('Gray');
  mersa_sheet_dest.setVerticalAlignment('Middle');
  mersa_sheet_dest.setHorizontalAlignment('Center');
  mersa_sheet_dest.setWrap(true);


  setFilter(mersa_google_sheets, "WOO_Processing");
}
...
//  const MERSA_INDEX = [MERSA_MAX_COL, MERSA_NOTE_COL, MERSA_MAX_ROW, MERSA_PROJECT_COL, MERSA_FNAME_COL, MERSA_LNAME_COL, MERSA_COMPANY_COL, MERSA_PHONE_COL, MERSA_CONDITION_COL, MERSA_COMPLETE_DATE_COL, MERSA_BILLING_COL, MERSA_NUMBER_OF_ORDERS, MERSA_CONDITION_COMPLETED];
  ...
  var mersa_edit_date_source = sh_order.getRange(1,sh_index[9],sh_index[2],1).getValues();
  ...
  sh_main.getRange(number_of_chosen_projects+1,6,1,1).setValue('تاریخ آخرین تغییر');
  ...
  for (let counter = 1; counter <= sh_index[11]; counter = counter + 1) {
    ...
    if(mersa_condition_source[counter][0] === sh_index[12]) {
      const edited_date = Utilities.formatDate(mersa_edit_date_source[counter][0], 'GMT-0700', 'yyyy-MM-dd\'T\'HH:mm:ss.SSS\'Z\'');
